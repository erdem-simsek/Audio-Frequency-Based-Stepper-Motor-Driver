...
Nokia_Init
	...
	;Setup GPIO
		...
		LDR 	R0, [R1]
		ORR		R0, #0xEC				; and make PA4 input
		STR		R0,[R1]
		...
		MOV32	R0, #0x00222200			; set 2,3,4 and 5 nibble			
		STR		R0,[R1]
	...
	;Setup SSI	
		...
		; small delay
		MOV		R0,#0x0F
settleSSIClk							; allow clock to settle
		SUBS	R0,R0,#0x01
		BNE		settleSSIClk

		LDR		R1,=SSI0_CR1			; disable SSI during setup and also set to Master
		MOV		R0,#0x00					; clear bit 1	and  clear bit 2 
		STR		R0,[R1]
...
waitSendNokia		
		...
		LDR		R0,[R1]
		ANDS	R0,R0,#0x02				;checking transmit fifo not full
		BEQ		waitSendNokia
		LDR		R1,=SSI0_DR
		STRB	R5,[R1]
		POP		{R0,R1}
